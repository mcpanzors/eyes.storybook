...
const rewriteStorybookConfig = (configPath, configBody) => {
  fs.writeFileSync(configPath, configBody, { encoding: 'utf8' });
};

const updateStorybookConfig = (configs, configBody) => {
  let templateName = 'storybook';
  if (configs.storybookVersion === 2) {
    templateName = 'storybook.v2';
  }

  const template = fs.readFileSync(`${__dirname}/configTemplates/${templateName}.js`, 'utf8');
...
class EyesStorybookUtils {
  ...
  static startServer(logger, promiseFactory, configs) {
    ...
    if (!storybookConfigBody.includes('__storybook_stories__')) {
      ...
      const template = updateStorybookConfig(configs, storybookConfigBody);
      rewriteStorybookConfig(storybookConfigPath, template);
    ...
    process.on('exit', () => {
      if (isConfigOverridden) {
        ...
        rewriteStorybookConfig(storybookConfigPath, storybookConfigBody);
      }
   ...
   * @param {string} resultsBody
   */
  static writeResultsFile(resultsPath, resultsBody) {
    fs.writeFileSync(path.resolve(process.cwd(), resultsPath), resultsBody, { encoding: 'utf8' });
  }
}
